...
angular.module('AppModule')
    .controller('SnapCont', ['$scope', 'generator', function ($scope, generator) {
        $scope.formData = {
            height: 16,
            width: 16,
        }
        ...
        $scope.Rclick = function (event, item) {

            if (!item.click) {
                if (angular.element(event.target).hasClass('flag')) {
                    angular.element(event.target).removeClass('flag');
                    angular.element(event.target).addClass('question');
                }
                else if (angular.element(event.target).hasClass('question')) {
                    angular.element(event.target).removeClass('question');
                }
                else {
